...
#
# Usage:
#   ./cache.sh start [--attach]   Start Redis
#   ./cache.sh stop               Stop Redis
#   ./cache.sh restart [--attach] Restart Redis
#   ./cache.sh status             Check if running
#   ./cache.sh logs [N] [--watch] Show logs

# Service configuration
SERVICE_NAME="dev-cache"
READY_PATTERN="Ready to accept connections"
SERVICE_CMD="cd /home/vscode/project && docker compose up redis"
...
do_stop() {
    ...
    (cd "$PROJECT_ROOT" && docker compose stop redis 2>/dev/null) || true
...
do_start() {
    ...
    local is_running
    is_running=$(cd "$PROJECT_ROOT" && docker compose ps --status running redis -q 2>/dev/null)

    if [[ -n "$is_running" ]]; then
        echo "[$SERVICE_NAME] Container already running, creating session..."
        tmux new-session -d -s "$SERVICE_NAME" -c "$PROJECT_ROOT" "docker compose logs -f redis"
